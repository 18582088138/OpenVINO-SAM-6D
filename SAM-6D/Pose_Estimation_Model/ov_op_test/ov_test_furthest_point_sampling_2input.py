...
torch_model_path = "./test_model/furthest_point_sampling_2.pth"
onnx_model_path = "./test_model/furthest_point_sampling_2.onnx"
ov_model_path = "./test_model/furthest_point_sampling_2.xml"

ov_kernel_path = "ov_op/furthest_point_sampling_2_cl.xml"
...
class FurthestPointSampling2(torch.autograd.Function):
    def __init__(self):
        super(FurthestPointSampling2, self).__init__()
    
    @staticmethod
    def forward(ctx, pts, npoint):
        npoint_i = npoint.shape[0]
        furthest_point_sampling = _ext.furthest_point_sampling(pts, npoint_i)
        ctx.save_for_backward(furthest_point_sampling)
        return furthest_point_sampling

    @staticmethod
    def symbolic(g: torch.Graph, pts: torch.Tensor, npoint: torch.Tensor) :
        return g.op("FurthestPointSampling2", pts,  npoint)

class MyModel(nn.Module):
    def __init__(self):
        super(MyModel, self).__init__()
        self.custom_furthest_point_sampling = FurthestPointSampling2.apply
        # self.npoint = 2048

    def forward(self, pts, npoint):
        '''
        pts: B*N*3
        npoint: 2048
        '''
        return self.custom_furthest_point_sampling(pts, npoint)


def get_input_data():
    dummy_pts = torch.randn(DUMMY_BATCH_SIZE, DUMMY_N, DUMMY_CHANNEL)
    # dyummy_nponts = torch.tensor(DUMMY_npints)
    dyummy_nponts = torch.ones(DUMMY_npints)

    onnx_input = (dummy_pts, dyummy_nponts)
    onnx_input_name = ["pts", "npoint"]
    ov_input = {"pts":dummy_pts, "npoint":dyummy_nponts}
...
def torch_ov_compare_cpu(ov_input, device="CPU"):
    ...
    torch_infer_result = torch_model(ov_input["pts"], ov_input["npoint"])
